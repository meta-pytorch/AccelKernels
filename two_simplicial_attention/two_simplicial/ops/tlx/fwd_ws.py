...
from two_simplicial.utils import get_two_simplicial_tensor_core_tflops
...
@triton.jit
def _two_simplicial_tlx_fwd_ws_kernel(
    desc_q,  # [b, s, k, h]
    desc_k1,  # [b, s, 1, h]
    desc_k2,  # [b, s, 1, h]
    desc_v1,  # [b, s, 1, h]
...
def two_simplicial_tlx_fwd_ws(
    ...
    _two_simplicial_tlx_fwd_ws_kernel[grid](
        desc_q,
        desc_k1,
        desc_k2,
        desc_v1,
...
def _run_bench(
    ...
    total_tensor_core_tflops = get_two_simplicial_tensor_core_tflops(
    ...
    fn = lambda: two_simplicial_tlx_fwd_ws(q, k1, k2, v1, v2, w1, w2)
    ...
    two_simplicial_tflops = tensor_core_tflops * 1.5

    del q, k1, k2, v1, v2

    return ms, two_simplicial_tflops, tensor_core_tflops
...
def _run_bench_fa3(bs, seq_len, num_heads, num_kv_heads, head_dim, w1, w2):
    ...
    def _total_tensor_tflops():
        ...
        # So we multiply 1.5 of tensor core TFlops to get the two_simplicial TFlops
...
def _bench():
    ...
    for w2 in w2s:
        for seq_len in seq_lens:
            ms, two_simplicial_tflops, tensor_core_tflops = _run_bench(
                bs, seq_len, num_heads, num_kv_heads, head_dim, w1, w2
            )
            ...
            print(
                f"{bs=} {seq_len=} {num_heads=} {head_dim=} {w1=} {w2=} | latency ms={ms:.3f} two_simplicial_tflops={two_simplicial_tflops:.3f} tensor_core_tflops={tensor_core_tflops:.3f} | latency ms (FA3)={ms_fa3:.3f} tensor_core_tflops (FA3)={tensor_core_tflops_fa3:.3f}"
            )
