...
def _bench():
    ...
    for w2 in w2s:
        for seq_len in seq_lens:
            ms, triplet_tflops, tensor_core_tflops = _run_bench(
                bs, seq_len, num_heads, num_kv_heads, head_dim, w1, w2
            )
            ...
            print(
                f"{bs=} {seq_len=} {num_heads=} {head_dim=} {w1=} {w2=} | latency ms={ms:.3f} triplet_tflops={triplet_tflops:.3f} tensor_core_tflops={tensor_core_tflops:.3f} | latency ms (FA3)={ms_fa3:.3f} tensor_core_tflops (FA3)={tensor_core_tflops_fa3:.3f}"
            )
